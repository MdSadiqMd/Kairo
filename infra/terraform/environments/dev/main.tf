# Dev environment root. Modules resolve in dependency order via references:
...
# (iam is placed after eks because its IRSA trust policies need the OIDC provider;
# Terraform's dependency graph enforces correct ordering regardless of file order.)
...
module "eks" {
  ...
  # No dedicated "secrets" CMK; reuse the general-purpose EBS CMK (rotation + root
  # admin) for EKS envelope encryption of Kubernetes secrets.
...
module "eventing" {
  ...
  name_prefix = var.name_prefix
  # Reuse the DynamoDB data CMK for Kinesis/SQS encryption (no dedicated stream CMK).
  kms_key_arn = module.kms.dynamodb_key_arn
}
...
module "aurora" {
  ...
  subnet_ids  = module.network.private_data_subnet_ids
  vpc_cidr    = var.vpc_cidr
  # Reuse the general-purpose EBS CMK for RDS storage encryption.
...
module "waf" {
  source      = "../../modules/waf"
  name_prefix = var.name_prefix
  # alb_arn is empty: the ALB is created by the AWS Load Balancer Controller during
  # the Kubernetes rollout; association is applied out-of-band there.
}
...
module "security_baseline" {
  ...
  # CloudTrail data events on the training-data buckets.
...
# API-key secret for the router (Secrets Manager everywhere). Named
# kairo-<env>-api-key to match the router config (ROUTER_API_KEYS_SECRET) and the
# `make api-key` helper. A random dev key is seeded here; production keys are set
# by rotation out-of-band, so secret_string changes are ignored after create.
...
resource "aws_secretsmanager_secret_version" "api_key" {
  ...
  lifecycle {
    ignore_changes = [secret_string] # real keys are rotated out-of-band
  }
}

# FSx for Lustre weight staging. Off by default in dev (priced, always-on);
# enable for staging/prod where large-model cold-start latency justifies it.
module "fsx_lustre" {
